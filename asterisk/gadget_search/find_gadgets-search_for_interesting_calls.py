...
class FindInterestingCalls(gdb.Command):
  ...
  def __init__ (self):
    super (FindGadgetsAroundCodePointer, self).__init__ ("find_interesting_calls",
                         gdb.COMMAND_SUPPORT,
                         gdb.COMPLETE_NONE)
  ...
  def check_for_interesting_instruction(self,addr,gadget):
    ...
    if disas.startswith("call") and "plt" in disas:
      print "%d|%x %s" % (len(gadget),addr, disas)
